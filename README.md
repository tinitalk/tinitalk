...
Новый сервер и новый Android-клиент не требуют собственного Firebase project,
...
Для нового сервера строка `webpush.vapid` должна иметь значение `ok`.
Статусы `fcm.*: missing` допустимы и не мешают новым клиентам.

### Совместимость со старыми APK

Сервер сохраняет поддержку старых клиентов, использующих FCM. Если она нужна на
новом сервере, один раз импортируй оба файла из одного Firebase project:
...
После импорта файлы можно удалить с сервера: закрытый service account и
разобранный Android-конфиг сохраняются в SQLite. Новый клиент их не запрашивает.
При обновлении уже настроенного сервера обычный `tinitalk init` добавляет
WebPush-ключи и не удаляет сохранённую FCM-конфигурацию.
...
- `/var/lib/tinitalk/state.db` должен принадлежать `tinitalk:tinitalk` и иметь
  mode `0600`.
- Не коммить legacy Firebase service account JSON, `state.db`, APK и собранные
  бинарники.
...
- APK не содержит Firebase JSON-файлов. В одном приложении можно добавить
...
- После обновления клиента со старой FCM-схемы выйди из сохранённых аккаунтов и
  войди заново, чтобы зарегистрировать для них WebPush-подписки.
