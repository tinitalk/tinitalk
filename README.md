...
- Перед обновлением с API 3 на API 4 останови сервис и создай backup командой
  `tinitalk backup` ещё старым бинарником; первый запуск нового бинарника
  необратимо обновит схему БД. Затем замени `/usr/local/bin/tinitalk`, запусти
  `doctor` от root пока низкие порты свободны и снова запусти сервис.
